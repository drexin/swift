...
swift_generic_instantiateLayoutString(const uint8_t* layoutStr,
  ...
  do {
    ...
    if (tag == 2) {
      ...
      if (genericType->getTypeContextDescriptor()->hasLayoutString()) {
        ...
        genericRefCountSize += readBytes<uint32_t>(genericLayoutStr, countOffset);
  ...
  const auto instancedLayoutStrSize = layoutStringHeaderSize + refCountSize + genericRefCountSize + 4 + 1;
  ...
  writeBytes<uint32_t>(instancedLayoutStr, 25, refCountSize + genericRefCountSize);
  ...
  do {
    ...
    if (tag == 1) {
      memcpy((void*)(layoutStr + layoutStrOffset), (void*)(instancedLayoutStr + instancedLayoutStrOffset), index);
      ...
      if (genericType->getTypeContextDescriptor()->hasLayoutString()) {
        const uint8_t *genericLayoutStr = genericType->getLayoutString();
        size_t countOffset = 25;
        auto genericRefCountSize = readBytes<uint32_t>(genericLayoutStr, countOffset);
        if (genericRefCountSize > 0) {
          memcpy((void*)(genericLayoutStr + layoutStringHeaderSize), (void*)(instancedLayoutStr + instancedLayoutStrOffset), genericRefCountSize);
          if (skipBytes) {
            size_t firstRCOffset = instancedLayoutStrOffset;
            auto firstRC = readBytes<uint32_t>(instancedLayoutStr, firstRCOffset);
            firstRC += skipBytes;
          ...
          skipBytes += readBytes<uint32_t>(genericLayoutStr, trailingBytesOffset);
        }
      } else if (genericType->isClassObject()) {
  ...
  if (skipBytes > 0) {
    writeBytes<uint32_t>(instancedLayoutStr, layoutStringHeaderSize + refCountSize + genericRefCountSize, skipBytes);
  }
